...
const Row = ({ index, style, list, focusedTrip, onFocusOnTrip, renderTripStat }: { index: number; style: React.CSSProperties } & CustomRowProps) => {
  const trip = list[index];
  const isSelected = focusedTrip === trip;

  console.log('Rendering row', index, 'style:', style);
  ...
  return (
    <div style={style} className="px-4 py-1.5">
      <div
        ...
        className={cn(
          "group relative flex flex-col gap-3 p-4 rounded-xl border transition-all cursor-pointer hover:shadow-md h-full",
          isSelected
        ...
        <div className="space-y-2 flex-1">
          <div className="flex gap-2 h-full">
            <div className="mt-1 flex flex-col items-center gap-1">
              ...
              <div className="w-0.5 flex-1 bg-border border-l border-dashed min-h-[10px]" />
              <div className="w-2 h-2 rounded-full bg-red-500 shrink-0" />
            </div>
            <div className="flex-1 space-y-3 text-sm">
              <div>
                <p className="text-xs text-muted-foreground mb-0.5">Pickup</p>
                <p className="font-medium leading-tight line-clamp-1" title={trip.begintrip_address}>
                  {trip.begintrip_address || 'N/A'}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground mb-0.5">Dropoff</p>
                <p className="font-medium leading-tight line-clamp-1" title={trip.dropoff_address}>
      ...
      <div className="flex-1 min-h-0" style={{ position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <List<CustomRowProps>
            ...
            rowHeight={220}
            rowProps={{ list, focusedTrip, onFocusOnTrip, renderTripStat }}
          />
        </div>
      </div>
    </div>
